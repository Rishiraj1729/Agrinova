% AgriNova --- NCSC Project Report (Form-A)
% Theme: Science and Innovation for Sustainability
% Group members: Shikha Sharma, Samriddhi Ghosh --- Class XI, KV Dum Dum
% PRINT NOTE: Designed for black-and-white photocopy --- black ink, white paper,
% hatching / bold rules instead of colour fills. Soft greys avoided for body text.
...
% --- B&W print palette (high contrast) ---
\definecolor{nvink}{gray}{0}
\definecolor{nvmuted}{gray}{0.22}
\definecolor{nvline}{gray}{0.55}
\definecolor{nvband}{gray}{0.92}
\definecolor{nvpaper}{gray}{1}
\definecolor{nvgreen}{gray}{0}
\definecolor{nvleaf}{gray}{0}
\definecolor{nvsaff}{gray}{0.15}

\hypersetup{
  colorlinks=false,
  pdfborder={0 0 0},
  pdftitle={AgriNova --- NCSC Form-A Project Report},
  pdfauthor={Shikha Sharma, Samriddhi Ghosh}
}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\bfseries NCSC Form-A \textbullet\ Science and Innovation for Sustainability}
\fancyhead[R]{\small\bfseries AgriNova}
\fancyfoot[C]{\small\thepage}
\renewcommand{\headrulewidth}{0.8pt}
\renewcommand{\footrulewidth}{0.4pt}

\titleformat{\section}{\large\bfseries}{\thesection}{0.8em}{}[\vspace{0.15em}{\titlerule[0.6pt]}]
...
\newenvironment{conv}[1]{%
  \par\vspace{0.55em}%
  {\small\bfseries #1}\par\vspace{0.2em}%
  \noindent{\vrule width 2.4pt}\hspace{0.65em}%
...
\vspace*{2mm}
\begin{center}
{\small\bfseries NATIONAL CHILDREN'S SCIENCE CONGRESS}\\[0.25em]
{\footnotesize Theme: \textit{Science and Innovation for Sustainability}}\\
{\footnotesize Sub-themes: Waste Management \textbullet\ Food, Agriculture \& Health}\\[0.35em]
{\footnotesize\bfseries Project Report --- Form-A (strict structure)}\\[0.9em]
{\LARGE\bfseries AGRINOVA}\\[0.3em]
{\large Turning crop residue into income --- not smoke, not drain waste}\\[0.4em]
{\small\itshape Predict.\ Protect.\ Prosper.\ Recycle.}\\[0.9em]
\begin{tabular}{@{}ll@{}}
...
\textbf{Field area} & Madhyamgram \& Barasat (doorstep) \textbullet\ Punjab by phone\\
\textbf{Live desk} & \url{https://agrinova-hazel.vercel.app}\\
...
\hrule height 1.1pt
\vspace{0.35em}
{\footnotesize\textit{Print note:} This report is set for black-and-white photocopy --- black text, bold rules, hatched diagrams. Soft colour fills are avoided so contrast survives a B\&W printout.}
\vspace{0.35em}
\hrule height 0.5pt
...
Work had two parts. Part~I mapped rice straw from published CRM, SWM and air-quality notes. Part~II is the Class~XI field book from Kendriya Vidyalaya Dum Dum. Ten farmers were closed in September~2025: six at the door in Madhyamgram and Barasat, starting at Hridaypur, and four in Punjab by phone with family contacts, plus two plant-floor calls and two local civic officers (Councillor, Ward~21, Barasat Municipality; Sanitary Inspector, Madhyamgram Municipality). AgriNova at \url{https://agrinova-hazel.vercel.app} was then used as a demonstration desk on those sheets. Default login and case-study data are West Bengal ward-first (Madhyamgram / Barasat ULB localities).
...
The gap is not another scheme pamphlet. It is a shared listing with moisture, buyer and pickup time. On this 10-farm book, about 72~t would move if the eight ``yes'' lots listed --- a cluster demonstration, not a state forecast. Carbon figures use the working constant 1.5~tCO$_2$e per tonne straw (estimate, not certified credits).
...
\begin{tikzpicture}[
  font=\small,
  box/.style={draw=black, line width=0.7pt, thick, align=center, inner sep=6pt, fill=white, text width=3.15cm},
  bad/.style={draw=black, line width=0.9pt, align=center, inner sep=6pt, pattern=north east lines, pattern color=black!35, text width=3.15cm},
  arr/.style={-{Latex}, thick, black}
]
...
\draw[arr] (n) -- (z);
\end{tikzpicture}
\caption{Problem sketch used to design the questionnaire. The missing object is the middle box --- not a new law. (Hatched boxes = failure modes; B\&W-safe.)}
\label{fig:problem}
\end{figure}
...
\toprule
\textbf{When} & \textbf{Where} & \textbf{Who} \\
\midrule
...
\begin{enumerate}
  ...
  \item Built AgriNova: \url{https://agrinova-hazel.vercel.app} (West Bengal ward defaults; India State / District / Ward login tabs).
  \item Demonstration walk-through on that desk using this book (Ramesh Das dump lot; Jaswinder Singh burn lot) plus curated leaf-disease sample images with known results on Kisan Bandhu.
\end{enumerate}
...
\begin{tikzpicture}[
  font=\footnotesize,
  d/.style={draw=black, line width=0.7pt, align=center, inner sep=7pt, fill=white, text width=2.55cm, minimum height=1.35cm},
  g/.style={draw=black, line width=0.9pt, align=center, inner sep=7pt, pattern=dots, pattern color=black!40, text width=2.55cm, minimum height=1.35cm},
  arr/.style={-{Latex}, thick, black}
]
...
\begin{figure}[h]
\centering
\includegraphics[width=0.92\textwidth]{11-landing.png}
\caption{Live landing desk (reference screenshot). Brand-first entry to West Bengal case study and role login. Source: AgriNova web app.}
\label{fig:landing}
\end{figure}
...
\toprule
\textbf{Name} & \textbf{Place} & \textbf{How} & \textbf{St} & \textbf{Ac} & \textbf{Fate} & \textbf{Phone} & \textbf{5d} \\
\midrule
...
\toprule
\textbf{Fate} & \textbf{PB phone} & \textbf{Local WB} & \textbf{All} & \textbf{Share} \\
\midrule
...
\begin{axis}[
  ...
  legend style={font=\small, at={(0.5,0.98)}, anchor=north, legend columns=2, draw=black, fill=white},
  enlarge x limits=0.22,
  grid=major,
  major grid style={black!25},
  axis line style={black},
]
\addplot[pattern=north east lines, pattern color=black, draw=black, thick] coordinates {(Burned,3) (Stacked,0) (Sold,1) (Mixed,0)};
\addplot[pattern=dots, pattern color=black, draw=black, thick] coordinates {(Burned,0) (Stacked,3) (Sold,1) (Mixed,2)};
\legend{Punjab (phone), Madhyamgram--Barasat}
\end{axis}
\end{tikzpicture}
\caption{Same straw, two failure modes on ten named sheets. Patterns (not colours) survive B\&W photocopy.}
\label{fig:fate}
\end{figure}
...
\toprule
\textbf{Name} & \textbf{Post} & \textbf{Gate} & \textbf{Spec} & \textbf{Small lots / listing} \\
\midrule
...
\toprule
\textbf{Name and post} & \textbf{Register today} & \textbf{Gap / desk} \\
\midrule
...
The survey is ten sheets. The software is a demonstration that those sheets can drive. Default case study and login desk are \textbf{West Bengal} (North 24 Parganas wards / ULB localities), with Punjab as the phone comparison belt.

On \url{https://agrinova-hazel.vercel.app} we listed Ramesh Das (3~ac Doltala, wet stack, $\approx$6~t) --- match preferred a short-haul compost spec, not a paper mill that would reject moisture. We listed Jaswinder Singh's phone sheet (6~ac, dry burn window, $\approx$12~t) --- match preferred a $\le$15\% biomass cabin. The same score, two clocks: dump here, burn there. The in-app Punjab case-study cluster (larger demo farms and plants) was used only as a \textbf{worked example} of distance, demand and moisture. It is labelled demonstration data. It is not extra interviews.

A judge can repeat both listings in five minutes: Farmer desk $\rightarrow$ sell residue $\rightarrow$ match card $\rightarrow$ pickup. Government desk shows cluster tonnes, not names. Kisan Bandhu ships three curated leaf samples (rice blast, bacterial leaf blight, wheat rust) with fixed educational results for the booth.

\subsubsection{Mathematical accuracy (working constants)}
All income and carbon figures on the desk use the same documented constants (also on the Science page of the app):

\begin{itemize}
  \item Rice straw yield working figure: $Y = 2.0$~t/acre (literature band about 1.8--2.4~t/acre).
  \item Wheat stubble working figure: $1.6$~t/acre.
  \item Estimated straw on a farm: $T = A \times Y$ (acres $\times$ yield). Example: Doltala $A=3$ $\Rightarrow$ $T=6.0$~t.
  \item Working gate price: $P = \mathrm{Rs}\,700$/t (before freight). Gross cash $C = T \times P$. Example: $6.0 \times 700 = \mathrm{Rs}\,4200$.
  \item Avoided CO$_2$e estimate: $E = T \times 1.5$~tCO$_2$e/t (IPCC-style order-of-magnitude when open burn is replaced by utilisation). Example: $6.0 \times 1.5 = 9.0$~tCO$_2$e. \textbf{Not} a Verra / certified credit.
  \item Match score weights (sum 100): Distance~22, Rating~22, Demand~18, Moisture~14, Pathway~12, Price~12. Moisture can zero a bad match.
\end{itemize}

On this book: 40.5~acres $\times$ 2.0 $= 81$~t total straw estimate; eight ``Yes'' lots cover 36~acres $\approx$ 72~t $\approx$ Rs~50{,}400 at Rs~700/t; burned pocket 27~t $\times$ 1.5 $\approx$ 40.5~tCO$_2$e as an estimate.

\begin{figure}[h]
\centering
\includegraphics[width=0.88\textwidth]{01-login.png}
\caption{Login with India-specific tabs: Identity / State--Ward / Land. Default desk: West Bengal, North 24 Parganas wards (Madhyamgram \& Barasat).}
\label{fig:login}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.88\textwidth]{13-wb.png}
\caption{West Bengal case-study hub --- peri-urban Madhyamgram--Barasat narrative used as the default region.}
\label{fig:wb}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.88\textwidth]{04-sell.png}
\caption{Sell / match desk used to list the Doltala wet stack and the Kharar burn-window lot.}
\label{fig:sell}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.88\textwidth]{05-kisan-ai.png}
\caption{Kisan Bandhu field desk --- residue advice, leaf scan, and curated sample results for the booth.}
\label{fig:bandhu}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.88\textwidth]{08-government.png}
\caption{Ward / ULB government desk --- utilised vs dumped aggregates without farmer phone numbers.}
\label{fig:govdesk}
\end{figure}

Completed sale writes avoided-CO$_2$e at about 1.5~tCO$_2$e per tonne straw (estimate, not Verra). Farmer credit / buyer CSR line are simulators.
...
\addplot[pattern=horizontal lines, pattern color=black, draw=black, thick] coordinates {(14,Moisture) (12,Pathway) (12,Price) (18,Demand) (22,Distance) (22,Rating)};
\end{axis}
\end{tikzpicture}
\caption{Explainable match --- moisture can zero a lot that would otherwise look close. Hatched bars for B\&W print.}
\label{fig:match}
\end{figure}
...
Live desk: \url{https://agrinova-hazel.vercel.app}.
